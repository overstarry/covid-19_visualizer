...
class Line {
  ...
  renderTransLineAndTooltip(x, y, bodyX) {
    ...
    // dot圆圈动画//*[@id="contracted"]/div[2]/svg/g[2]/circle[1]document.querySelector("")
    //#contracted > div.c-chart1.main844560 > svg > g.body > circle.dot._0.index_0
    d3.selectAll('div.c-chart1> svg > g.body >circle.dot').transition().duration(100).ease(d3.easeCubicOut).attr('r', 2)
    d3.selectAll('div.c-chart1> svg > g.body >circle.index_' + cutIndex).transition().duration(100).ease(d3.easeBounceOut).attr('r', this._activeR)
    //提示框位置和内容
    if (x > this.quadrantWidth() - this._tooltip.style('width').slice(0, -2) - this._padding * 2) {
      x = x - this._tooltip.style('width').slice(0, -2) - this._padding * 2 - this._offset * 2 + 150;
    }
